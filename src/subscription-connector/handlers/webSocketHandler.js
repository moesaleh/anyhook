...
class WebSocketHandler extends BaseHandler {
    connect(subscription) {
        ...
        wsClient.on('message', (message) => {
            ...
            console.log(`WebSocket message received for subscription ID: ${subscription_id}`, decodedMessage);
            ...
            if (event_type && parsedMessage.event !== event_type) {
                console.log(`Event '${parsedMessage.event}' did not match expected '${event_type}' for subscription ID: ${subscription_id}`);
            } else {
        ...
        wsClient.on('close', (code, reason) => {
            console.log(`WebSocket connection closed for subscription ID: ${subscription_id}`, code, reason);
        });

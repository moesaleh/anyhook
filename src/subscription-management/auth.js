...
function mountAuthRoutes(app, { pool }) {
    ...
    app.post('/organizations', requireAuth, requireRole('owner', 'admin', 'member'), async (req, res) => {
        if (req.auth.via !== 'cookie') {
    ...
    app.post('/organizations/current/members', requireAuth, requireRole('owner', 'admin'), async (req, res) => {
        ...
        try {
            ...
            if (userResult.rowCount === 0) {
                return res.status(404).json({ error: 'No user with that email; they must register first' });
            }
            await pool.query(
                `INSERT INTO memberships (user_id, organization_id, role)
                 VALUES ($1, $2, $3)
    ...
    app.delete('/organizations/current/members/:userId', requireAuth, requireRole('owner', 'admin'), async (req, res) => {
    ...
    app.post('/organizations/current/api-keys', requireAuth, requireRole('owner', 'admin'), async (req, res) => {
    ...
    app.delete('/organizations/current/api-keys/:id', requireAuth, requireRole('owner', 'admin'), async (req, res) => {
        try {
            const result = await pool.query(
